namespace PluralVideos.Decrypt.Encryption
{
    public class VideoEncryption
    {
        ...
        public static void XorBuffer(byte[] buff, int length, long position)
        {
            ...
            {
                byte num = (byte)((ulong)(str1[(int)((position + index) % str1.Length)] ^ str2[(int)((position + index) % str2.Length)]) ^ (ulong)((position + index) % 251L));
        ...
        public static void XorBufferV2(byte[] buff, int length, long position)
        {
            for (int index = 0; index < length; ++index)
            {
                byte num = (byte)((ulong)(string1_v2[(int)((position + index) % string1_v2.Length)] ^ string2_v2[(int)((position + index) % string2_v2.Length)]) ^ (ulong)((position + index) % 251L));
                buff[index] = (byte)(buff[index] ^ (uint)num);
            }
        }
